...
int main(int argc, char** argv) {
    ...
    if (key == "-t") {
        ...
        IClassifier<regression::pair_ii, double>* solver =
                                new regression::RegressionSolver();
        int x, y;
        double label;
        char c;
        std::vector<std::pair<regression::pair_ii, double> > samples;
        while (in.good()) {
            in >> x >> c >> y >> c >> label;
        ...
        if (!solver->train(samples)) {
            ...
            for (size_t i = 0; i < samples.size(); ++i) {
                double val = solver->classify(samples[i].first);
                error_int += sqrt((samples[i].second - val) * (samples[i].second - val));
            }
            std::cout << "Internal training error = "
                      << error_int / samples.size() << std::endl;
        }
        delete solver;
        return 0;
    }
    if (key == "-s") {
        ...
        while (io.getInput().good()) {
            ...
            ss >> x >> c >> y;
            int label = solver->classify(std::make_pair(x, y));
